...
function FormMedia() {
  ...
  const ENDPOINT: string = process.env.NEXT_PUBLIC_API_ENDPOINT || 'http://localhost:5000/v1';
  ...
  const [url, setUrl] = useState(`http://localhost:5000/v1/media/photos`);
  ...
  const onChangeType = (type: any, props: FormikProps<FormValues>) => {
    ...
    setFileUpload(null);
    setUrl(`http://localhost:5000/v1/media/${type.currentTarget.value}s`);
  };
